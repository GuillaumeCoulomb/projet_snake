...
def add_player_to_yaml(file_path, player_name, score):
    try:
    ...
    if any(player.get('name') == player_name for player in data):
        print(f"Le joueur {player_name} existe déjà dans le fichier.")
        return
                ...
                case State.GAMEOVER :
                    ...
                    if countdown==0 :
                        ...
                        if self._scores.is_highscore(score) is True :
                            ...
                            add_player_to_yaml('snake_scores.yml', 'nouveau_joueur', 15)
